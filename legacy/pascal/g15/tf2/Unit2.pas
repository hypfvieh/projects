...
begin
MessageDlg('Team Fortress 2 - Statsplugin' +#10#13+
...
ini.WriteInteger('Position','SERVER',formatting.Items.IndexOf('IP: 1.2.3.4'));
ini.WriteInteger('Position','TITLE',formatting.Items.IndexOf('TF2: Source - Statsplugin'));
...
begin
     ...
     ini := tinifile.Create(extractfilepath(application.exename) + 'settings.ini');
      ...
      formatting.clear;
      formatting.items.Add('TF2: Source - Statsplugin');
      formatting.items.Add('IP: 1.2.3.4');
      ...
      for j := 0 to formatting.items.Count - 1 do begin
        ...
        if formatting.items.Strings[j] = 'Map: dod_sample' then
          formatting.Items.Move(j,ini.readinteger('Position','MAP',2));

        if formatting.items.strings[j] = 'IP: 1.2.3.4' then
          formatting.items.move(j,ini.ReadInteger('Position','SERVER',1));

        if formatting.Items.Strings[j] = 'TF2: Source - Statsplugin' then
